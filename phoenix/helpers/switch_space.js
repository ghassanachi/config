...
function switchSpace(modifier, wrap = SPACES_SWITCH_WRAP) {
    ...
    if (nextIndex < 0 || nextIndex >= spaces.length) {
        ...
        nextIndex =
      nextIndex < 0 ? spaces.length + nextIndex : nextIndex % spaces.length;
    }

    const script = `tell application "System Events" to key code {${index2keycode(
        nextIndex
    )}} using {control down, option down, command down, shift down}`;

    setTimeout(() => {
        return osascript(script);
    }, 250); // FIXME: https://github.com/kasper/phoenix/issues/206
}

...
function writeList(callback = _.noop) {
    ...
    spacesList.items = spacesList.items.filter((item) => {
        return item.screens_hashes.includes(screen_hash);
    }
    );
...
function updateSpaceCycle(
    iteration = cycleIteration,
    interval = SPACES_UPDATE_INTERVAL,
    limit = SPACES_UPDATE_CYCLES
) {
    if (iteration === 0) {
    // Iterations start
        ...
        cycleInterval = setInterval(
            () => {
                return updateSpaceCycle(undefined, undefined, limit);
            },
            interval
        );
    }

    if (cycleIteration >= limit) {
    // Iteration end
...
function updateSpaces() {
    ...
    list.items = spaces.map((space, index) => {
        ...
        const prevItem = spacesListRaw.items.find(
            (item) => {
                return item.space_hash === space_hash;
            }
        );
    ...
    if (_.isEqual(list, spacesListRaw)) {
        return;
    }
...
function updateSpace(space, index, screens) {
    ...
    if (!Space.active().isEqual(space)) {
        return false;
    } // We can't get windows from inactive spaces
    ...
    if (_.isEqual(spacesListRaw.items[index], item)) {
        return false;
    }
...
function getSpaceItem(space, index, screens) {
    return {
        ...
        screens_hashes: screens.map((screen) => {
            return screen.hash();
        }),
    };
}
